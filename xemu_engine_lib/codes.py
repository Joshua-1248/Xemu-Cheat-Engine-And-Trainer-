...
class CheatEngine:
    ...
    def _typeD(self, codes, idx, cmd, val, code_type, mem_file):
        ...
        Type 0xD - conditional. Guards the next N lines.
            ...
            Daaaaaaa NNxTsvvv    a = 28-bit offset
        ...
        is retired; D expresses everything it could. To convert an old E code:
            ...
            D_val = (n << 24) | (test << 20) | (is_8bit << 19) | compare
        """
        n = self._d_n(cmd, val, code_type)
        offset = cmd & 0x0FFFFFFF
    ...
    def _typeE(self, codes, idx, cmd, val, mem_file, bid):
        ...
        Type 0xE - conditional ON/OFF SWITCH. Guards the next N lines.
            ...
            Eaaaaaaa NNiTsvvv    a = 28-bit offset
                                 v = 16-bit compare value
                                 s = 8-bit flag (bit 19)
                                 T = test (bits 20-22)
                                 i = initial state (bit 23): 0 off, 1 on
                                 N = lines guarded (bits 24-31)

        Field layout is identical to type D, so converting a D code to a switch
        is a single nibble in the type. Tests are the same eight.
        ...
        test = (val >> 20) & 0x7
        is_8bit = ((val >> 19) & 1) == 1
        initial_on = ((val >> 23) & 1) == 1

        key = (bid, idx)
        st = self._switches.get(key)
        if st is None:
            st = {'on': initial_on, 'prev': False}
            self._switches[key] = st

        mem_val = self._r8(offset, mem_file) if is_8bit \
            else self._r16(offset, mem_file)
        if mem_val is None:
